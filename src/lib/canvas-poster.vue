...
export default {
  ...
  methods: {
    ...
    // 数组转字符串，并且去掉不需要的字符
    removeListWord(wordList) {
      let _tempStrList = [];
      for (let i = 0; i < wordList.length; i++) {
        if (wordList[i] !== "<blod>" && wordList[i] !== "</blod>") {
          _tempStrList.push(wordList[i]);
        }
      }
      return _tempStrList.join("");
    },
    // 开始绘图
    ...
    drawText(params) {
      ...
      that.ctx.textAlign = textAlign;
      that.ctx.fillStyle = color;
      //  this.ctx.font = "normal 36px Arial";
      that.ctx.font = `normal normal ${fontSize}px Arial`;
      //  top + fontSize  解决 this.ctx.textBaseline = 'top'问题
      const textTop = top + fontSize;

      if (!breakWord) {
        that.ctx.fillText(content, left, textTop);
        that.drawTextLine(
          left,
          textTop,
          textDecoration,
        ...
        for (let i = 0; i < content.length; i++) {
          if (fillText.length === 0) {
            if (_tempWord !== "" && _tempWord !== undefined) {
              fillText.push(_tempWord);
            }
          }
          // TODO 这里需要去除<blod>和</blod>所占据的长度，因为这些不会显示，只是一个标记
          let _tempText = that.removeListWord(fillText);

          if (that.ctx.measureText(_tempText + [content[i]]).width > width) {
            _tempWord = content[i];
            if (lineNum === MaxLineNumber) {
              if (i !== content.length) {
                fillText.pop();
                fillText.push["..."];
                that.ctx.fillText(fillText.join(""), left, fillTop);
                that.drawTextLine(
                  left,
                  fillTop,
                  textDecoration,
            ...
            // 需要绘制的文字列表，这里面没有<blod>标签
            let tempDrawText = [];

            // 当前 x轴位置
            let xCoordinate = left;

            for (let i = 0; i < fillText.length; i++) {
              if (fillText[i] === "<blod>" || fillText[i] === "</blod>") {
                if (tempDrawText.length === 0) {
                  if (fillText[i] === "<blod>") {
                    that.ctx.font = `normal bolder ${fontSize}px Arial`;
                  }
                  if (fillText[i] === "</blod>") {
                    that.ctx.font = `normal normal ${fontSize}px Arial`;
                  }
                ...
                that.ctx.fillText(tempDrawText.join(""), xCoordinate, fillTop);
                xCoordinate += that.ctx.measureText(tempDrawText.join(""))
                  .width;
                tempDrawText = [];
        ...
        for (let i = 0; i < fillText.length; i++) {
          if (fillText[i] === "<blod>" || fillText[i] === "</blod>") {
            ...
            that.ctx.fillText(tempDrawText.join(""), xCoordinate, fillTop);
            xCoordinate += that.ctx.measureText(tempDrawText.join("")).width;
            tempDrawText = [];
